...
### 2026-08-21 v0.3.3〜v0.3.4: Aティア限定「頭固め/ヒモ荒れ」アラート + 「A＋」勝負時エフェクト
- 実測データ(shadow-data、n=87、Aティアのみ)を分析し、Aティア内でも成績が良かった2条件を特定：
  - 「頭固め」= 1号艇がA1級（賭け金の75%相当がr1勝利に乗っている実質頭固定の買い目構造と整合）
  - 「ヒモ荒れ」= 場の当地勝率スプレッド(最大-最小)が4.11〜5.64（接戦でも一強でもない帯で的中率50%・回収率166.8%が最良だった）
  - 「一般戦か」は当初候補に入れたが、Aティア87件中89.7%が一般戦(grade_number=5)でほぼ常時真になり判別力が無いため、通常表示からは外し「特別開催週(grade≠5)のため参考度低め」という例外注意表示のみに変更
- v0.3.3: 上記2条件をTIER JUDGEマスに個別アラート表示として追加（実装時、`race`オブジェクトが`grade_number`を保持していなかった不備に気づき`gradeNumber`保存を追加）
- v0.3.4: 両条件が同時に成立する状況(n=87中8件、9.2%)を「A＋ 👑」表示（金色グラデ枠・発光・専用glyph）に格上げ。このとき個別の2アラート行は表示せず統合。実装時、グラデーションテキスト塗り(`-webkit-text-fill-color: transparent`)が絵文字ごと不可視化する問題が見つかり、王冠だけ`.crown`クラスで塗りを解除する対応を追加
- **注意**：headFirm・himoRoughは`confidence`(ティア判定の元)を構成する7変数の一部(loc_win等)だけを見ており、confidenceとは別経路の判定のため、両条件が揃ってもtierがAにならない例が実測で1/9件存在する（headFirm&himoRough合致9件中B:1件）。現状はtier==='A'の時のみ判定・表示する設計のため、この1件では表示自体が出ない
- **事故と教訓**: この回のpush作業で、ローカル`master`(生データ`toda_races.json`含む非公開履歴)を誤って`git push origin master`し、公開GitHubリポジトリに生データ入り履歴を一時的に露出させる事故が発生。`gh api -X DELETE`で即時復旧。以後の公開手順はmemory `feedback_jzirtboat_push_checklist` に固定（master はローカル限定、公開は`--orphan`で作り直したブランチを`origin/main`へforce push）
- 実ブラウザ(Android実機)での見た目は未確認

### 2026-08-21 v0.3.5: 王冠絵文字を削除・レース一覧ミニ表示にもA＋反映
- 実機確認(8/16 R7)でA＋演出は正常表示されたが、王冠が過剰との判断で削除し「A＋」の文字のみに簡素化（`.tier-glyph .crown`のグラデ塗り解除CSSも不要になり削除）
- レース一覧（`renderRaceGrid`のミニバッジ）でも、詳細画面と同じ条件（tier==='A' && chanceAlerts().headFirm && himoRough）でA＋を表示するよう追加。`tier-mini.supreme`クラスで金色文字に
